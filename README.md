...
| 3 | `interfaces` | Editor | One folder per interface, one JSON file per component (`<interfaceId>/<componentId>.json` — no screenshot). Component tree, per-type field editing, CS2 script hooks shown as raw editable arg lists, live 2D layout preview (ports the client's aspect-ratio layout algorithm); MODEL components preview on demand via the existing Three.js model viewer. Several dumper/encoder bugs found and fixed this pass, including a shared cp1252 string-codec bug and a dump-writer charset bug — **100% byte-identical round-trip** (see TODO for details) |
| 4 | `sound_effects` | Editor | RS2's additive-synthesis sound-effect format (up to 10 simultaneous oscillator-bank "instruments" per effect, each with envelopes, an optional biquad filter, echo) — full field editor plus the real dumped WAV preview (rendered by cryogen's own synth engine). Dumper already existed; this pass added `encode()`/`getActions()` (previously a stub) — **100% byte-identical round-trip** (10,084/10,084). Live in-browser re-synthesis after edits isn't implemented yet — the WAV preview goes stale until the next dump (see TODO) |
...
| 15 | `sound_effects_midi` | Not started | Investigated this pass — despite the name, it's neither the `sound_effects` synthesis format nor the `music` compact-MIDI format; it's a distinct instrument voice/keymap structure (`Node_Sub14` in the client) with null-terminated string fields and a per-note sample-zone table. Not yet decoded (see TODO) |
