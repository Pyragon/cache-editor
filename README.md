...
| 32 | `bas` | Editor | Base animation sets (render anims) — movement-sequence matrix (stand/walk/run/teleport × direction/turn variants, each with a jump link to the animation), random stand sequences, model/rotation-physics fields. Dumper + repack added to cryogen (`BASDefinitions`, fields renamed per darkan `BasType`) — 2,574/2,574 functional round-trip (65% byte-identical; the rest is opcode-order variance in the original data) |
