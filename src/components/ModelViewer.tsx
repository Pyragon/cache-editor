...
export default function ModelViewer({ data, display, world, posedVertices, cameraStateRef, statsExtra, fitScale = 2.5, hideHeader, poseBounds, highlightVertices }: Props) {
  ...
  useEffect(() => {
    ...
    const { vertexCount, faceCount, vertexX, vertexY, vertexZ,
            triangleX, triangleY, triangleZ, faceColor, faceTextures } = data
    ...
    const writeUVs = makeUVWriter(data)
    const uvTest = new Float32Array(6)
    ...
    for (let f = 0; f < faceCount; f++) {
      ...
      // Degenerate mapping (non-finite UVs, the client's zero-scale NaN case):
      // the client draws it as an invisible smear — drop the face.
      if (tex >= 0 && !writeUVs(f, ia, ib, ic, uvTest, 0)) continue
