...
Within those repos there's a hierarchy: cryogen's cache classes were written before the cache was well understood, so their field names can be wrong or obfuscated; **`darkan-bot-refactor` is authoritative** (config decoders under `src/main/kotlin/com/jagex/game/runetek5/config/`). Before building an editor page for a new cache entry, always diff the cryogen dumper's fields opcode-by-opcode against the matching darkan-bot-refactor class, rename cryogen's wrong/obfuscated fields to the darkan names (the user re-dumps afterward), and only keep a differing cryogen name when it's both semantically correct and load-bearing in server code. Two cautionary examples: varp `defaultValue` was actually opcode 5 `clientCode`, and NPC opcode 134 "animations" were actually sound effects.
