...
Loaders are registered by cache-folder name in `src/loaders/index.ts`'s `registry` object; `getLoader(name)` looks one up. Many entries listed in `README.md`'s table don't have a dedicated loader yet — those fall back to the generic helpers in `src/loaders/common.ts`:
