...
- **Canvas drag MOVES only — no resize handles yet.** Dragging writes `basePositionX/Y` back through the component's aspect mode (`positionFromLocal`), which matters: mode 2 measures from the right/bottom edge so dragging right *decreases* the stored number, and modes 3–5 are 1/16384-of-parent fractions rather than pixels. Resizing wants the same treatment for `baseWidth/Height` against `aspectWidthType/HeightType` (mode 1 = parent minus base, mode 2 = fraction, mode 4 = aspect-ratio locked).
- **Every drag frame repaints the whole base canvas**, since the component genuinely moved. On a 459-component interface that may stutter; the paint is async and cancel-guarded so it degrades to paint throughput rather than queueing. If it bites, the fix is incremental/dirty-region drawing, not the overlay trick that fixed selection.
- **Snapping targets** are the parent's edges, those edges inset by `SNAP_MARGIN` (10px), the parent's centre, and every sibling's edges and centre. No grid, no equal-spacing distribution, and nothing outside the parent. The 10px inset is a guess at a common margin — if real interfaces cluster on a different number, change `SNAP_MARGIN`.
