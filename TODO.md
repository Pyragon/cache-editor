...
- **EDITOR SURFACES for things we've traced but only render (2026-07-25).** The renderer now understands a lot that the editor can't touch. Each of these needs a UI:
  - **Loc/object side panel in the 3D map**: show the selected loc's models and, per model, the **textures** each face group uses — with the ability to change them. The Lumbridge fireplace is the worked example: its flame is texture 110 and nothing in the UI says so, let alone lets you swap it.
  - **Per-texture material fields**: `blendType`/`effectCombiner` (0 opaque, 1 cutout, 2 alpha), `effectId`, `hdr` and the HDR op-graph multiplier (`1 + hdrOp*31/4096`, up to 32x), `textureSpeedU/V`. All drive rendering and none are editable.
  - **Region environment editor**: the map-environment record (`map_environments/<id>.json`) — sun colour/ambient/light/backlight/position, fog colour+depth, skybox, and the **bloom parameters from opcode 2** (`bloomThreshold`, `bloomStrength`, `whitePoint`, each a byte*8/255 so 0..8). We decode and use these now; there's no way to edit or repack them.
  - **Face-level data**: `facePriorities` (transparent draw order) and per-face alpha, both of which visibly change rendering.
- **Clone button on locs.** The 3D map's loc panel can edit and delete a placement but not duplicate one — add a Clone that copies the selected loc (id/shape/rotation/plane) onto a new tile, so repeated scenery doesn't have to be re-placed by hand.
